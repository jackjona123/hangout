...
function execute() {
  ...
  client.once("ready", async () => {
    ...
    const JJH = client.guilds.cache.get(JJH)
    if (!JJH) {
      throw Error("Bot is not in JJH, cannot fetch users")
    }
    const members = await JJH.members.fetch()
